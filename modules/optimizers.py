...
def build_optimizer(args, model):
    ...
    d_params = list(map(id, model.discriminator.parameters()))
    ed_params = filter(lambda x: id(x) not in (ve_params + d_params), model.parameters())
    ...
    optimizer_d = getattr(torch.optim, args.optim)(
...
def build_lr_scheduler(args, optimizers):
    if type(optimizers) is list:
        ...
        if args.lr_scheduler == 'StepLR':
            lr_scheduler = [getattr(torch.optim.lr_scheduler, args.lr_scheduler)(optimizer, args.step_size, args.gamma),
                            getattr(torch.optim.lr_scheduler, args.lr_scheduler)(optimizer_d, args.step_size, args.gamma)]
        else:
            ...
            lr_scheduler = [getattr(torch.optim.lr_scheduler, args.lr_scheduler)(optimizer, max_lr=5e-4, epochs=args.epochs, steps_per_epoch=14867, pct_start=0.01, anneal_strategy='cos'),
                            getattr(torch.optim.lr_scheduler, args.lr_scheduler)(optimizer_d, max_lr=5e-4, epochs=args.epochs, steps_per_epoch=14867, pct_start=0.01, anneal_strategy='cos')]
    else:
        if args.lr_scheduler == 'StepLR':
            lr_scheduler = getattr(torch.optim.lr_scheduler, args.lr_scheduler)(optimizers, args.step_size, args.gamma)
        else:
            # can be used later for OneCycleLR
            lr_scheduler = getattr(torch.optim.lr_scheduler, args.lr_scheduler)(optimizers, max_lr=5e-4, epochs=args.epochs, steps_per_epoch=14867, pct_start=0.01, anneal_strategy='cos')
    return lr_scheduler
